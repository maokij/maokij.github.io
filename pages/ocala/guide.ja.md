...
また名前のあるデータは、定数式内で `sizeof` 関数でサイズを取得できます。
...
data byte = [0]    // OK: same as `data [1]byte`
data []byte = [0]  // OK: same as `data [1]byte`
data [1]byte = [0] // OK
```
...
データの配置セクションまたは配置アドレスを指定できます。
省略時のデフォルトは現在のセクションです。
特別なアドレス `<reserved>` を指定すると、データの配置を予約できます。
予約されたデータは主に自己書き換えに利用します。
`*patch*` ディレクティブで後から設定できます。
